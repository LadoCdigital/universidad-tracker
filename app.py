...
from flask import Flask, render_template, redirect, url_for, flash, request, abort, send_file, jsonify, make_response, send_from_directory
...
# Función auxiliar para subir archivos a Supabase
def upload_file_to_supabase(file_storage, folder, user_id):
    """Sube un archivo a Supabase Storage y devuelve la ruta pública."""
    try:
        timestamp = datetime.now().timestamp()
        filename = f"{folder}/{user_id}_{timestamp}_{secure_filename(file_storage.filename)}"
        ...
        if hasattr(response, 'error') and response.error:
            print(f"Error subiendo a Supabase: {response.error}")
            return None

        return filename
    except Exception as e:
        print(f"Excepción en upload_file_to_supabase: {e}")
        return None

# Ruta para servir archivos (redirige a Supabase)
@app.route('/uploads/<path:filename>')
def uploaded_file(filename):
    """Redirige a la URL pública de Supabase."""
    public_url = supabase.storage.from_(bucket_name).get_public_url(filename)
    return redirect(public_url)

# Crear tablas y carpetas necesarias al inicio
...
# Decorador para administradores (ejemplo simple)
...
@scheduler.task('cron', id='daily_backup', day='*', hour=3, minute=0)
def daily_backup():
    backup_database()
...
def profile():
    ...
    if form.validate_on_submit():
        ...
        if form.profile_pic.data:
            ...
            file_path = upload_file_to_supabase(form.profile_pic.data, 'profile_pics', current_user.id)
            if file_path:
...
def new_subject():
    ...
    if form.validate_on_submit():
        ...
        # Guardar archivos con Supabase
        if form.study_plan.data:
            file_path = upload_file_to_supabase(form.study_plan.data, 'study_plans', current_user.id)
            if file_path:
                subject.study_plan = file_path
        if form.course_program.data:
            file_path = upload_file_to_supabase(form.course_program.data, 'course_programs', current_user.id)
            if file_path:
                subject.course_program = file_path
        if form.exam_program.data:
            file_path = upload_file_to_supabase(form.exam_program.data, 'exam_programs', current_user.id)
            if file_path:
                subject.exam_program = file_path
...
def upload_material():
    ...
    if form.validate_on_submit():
        file_path = upload_file_to_supabase(form.file.data, 'materials', current_user.id)
        if file_path:
            material = Material(filename=file_path, description=form.description.data,
...
def delete_material(id):
    ...
    if material.user_id != current_user.id:
        abort(403)
    # Opcional: eliminar archivo de Supabase
    try:
        supabase.storage.from_(bucket_name).remove([material.filename])
    except:
        pass
...
def google_calendar_events():
    ...
    calendar_id = 'es.ar#holiday@group.v.calendar.google.com'
    if not api_key:
